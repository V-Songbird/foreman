...
| `areaNotes` | Keep one-sentence lessons closed tasks recorded about a code area, and quote the relevant ones back when a later task plans to touch the same files: `{enabled}`. Off by default. Stored in `.foreman/notes.jsonl`, append-only, written only by the CLI. Every quoted line says how stale it is. Opening a file a lesson names surfaces it there too. A lesson that proved wrong is retired during a survey, and never quoted again. |
