...
// [Foreman: 303] Contract tests exercise actual assembly and the public CLI.
// These prove payload/rendering behavior, not that a Codex session waits.
// Reviewed increments are a Codex feature: every handoff and gate call here
// names host codex.
...
function input(rows, options = {}) {
  return {
    entry: '001', host: 'codex', destination: 'task', split: true,
...
for (const [name, row, runs, looks] of [
  ...
  test(`${name} verification is one valid increment with only its required checks`, (t) => {
    ...
    const gate = checkPrompt(result.prompt, { root, host: 'codex', destination: 'task', entry: '001', profile: result.profile });
