...
// [Foreman: 304] These tests prove transport and the real notes/checkpoint
// CLI contract. A separate Codex rehearsal must demonstrate actual waiting.
// Reviewed increments are a Codex feature: every handoff is assembled for host
// codex, and the Codex lifecycle adapter runs with FOREMAN_HOST=codex.
...
function input(destination, count = 1) {
  return {
    entry: '001', host: 'codex', destination, split: true, reviewEachIncrement: true,
...
test('review notes preserve feedback and decisions without closing the parent or unblocking dependents', (t) => {
  ...
  const started = runNodeScript(path.join(HOOKS_DIR, 'codex-task.js'), ['start', '--id', '002'], null, { FOREMAN_HOST: 'codex', FOREMAN_PROJECT_DIR: root });
