...
// Every call names its host. The investigation and decision rules are shared,
// so those tests also run for Claude Code, asserting what both hosts emit.
...
// Codex unless the overrides name another host.
function inputFor(mode, overrides = {}) {
  ...
  return {
    ...identity, host: 'codex', destination: 'clipboard',
    judgment: {
...
for (const host of HOSTS) {
  test(`decision intent rejects implementation test mutation without changing project data (${host})`, () => {
    const root = fixture();
    const input = inputFor('entryless', { host, kind: 'decision' });
...
describe('investigation intent survives assembly', () => {
  for (const host of HOSTS) {
    for (const mode of ['entry', 'entryless']) {
      for (const hasCheck of [false, true]) {
        test(`${mode} investigation ${hasCheck ? 'with a runnable check' : 'without runnable checks'} remains findings-only (${host})`, () => {
          const root = fixture();
          const input = inputFor(mode, { host });
          ...
          assert.doesNotMatch(result.prompt, /^Implement:/m);
          assert.ok(!result.prompt.includes(FIX_CEILING_SENTENCE));
          assert.doesNotMatch(result.prompt, /fix and re-run/);
          // Neither host's plan names an implementation step for an
          // investigation, and neither entry paragraph stages or commits.
          assert.doesNotMatch(result.prompt, /Make the change/);
          assert.doesNotMatch(result.prompt, /safe-commit\.js|staged:true/);
          if (host === 'claude' && mode === 'entry') {
            assert.match(result.prompt, /This task is an investigation: its only project writes are this roadmap bookkeeping/);
          }
          if (hasCheck) {
    ...
    test(`a user-authored investigation request is retained verbatim (${host})`, () => {
      const root = fixture();
      const request = 'Compare the retry branches and return a file-cited explanation for the operator.';
      const result = assemble(root, inputFor('entry', { host, request }));
      assertAccepted(result);
    ...
    test(`a pure investigation rejects test-first mutation before changing project data (${host})`, () => {
      const root = fixture();
      const input = inputFor('entry', { host });
...
describe('diagnostic checks and implementation checkpoints stay distinct', () => {
  ...
  for (const mode of ['entry', 'entryless']) {
    for (const host of HOSTS) {
      test(`${mode} clipboard research retains two checks without starting a commit protocol (${host})`, () => {
        const root = fixture();
        const input = inputFor(mode, { host });
        ...
        assert.doesNotMatch(result.prompt, /Checkpoint protocol|fix and re-run/);
        assert.ok(!result.prompt.includes(FIX_CEILING_SENTENCE));
        assert.doesNotMatch(result.prompt, /safe-commit\.js|git add/);
        assertDataUnchanged(root, before);
      });
    }
  ...
  for (const host of HOSTS) {
    for (const mode of ['entry', 'entryless']) {
      for (const checkCount of [1, 2]) {
        test(`${mode} decision with ${checkCount} checks does not authorize implementation repair (${host})`, () => {
          const root = fixture({}, { kind: 'decision', title: 'Choose the terminal error retry policy' });
          const input = inputFor(mode, { host, kind: 'decision' });
  ...
  for (const destination of ['task', 'agent', 'clipboard']) {
    for (const profile of ['standard', 'reinforced']) {
      test(`${destination}, ${profile}: runtime stays enforced while configured omissions and data are preserved`, () => {
        ...
        const legacy = checkPrompt(removed, { root, host: 'codex', destination, profile });
        ...
        const rejected = checkPrompt(altered, { root, host: 'codex', destination, profile });
