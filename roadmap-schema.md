...
each line is valid JSON.** `add`, `update-status`, `annotate`, and
`update-deps` all run the resulting file through the same checks `doctor`
reports, and the write is refused (`{"ok":false,"error":"refusing to write
ROADMAP.jsonl …"}`, exit 1, file untouched) if it would carry a structural
...
- `foreman:roadmap` — `next-candidates` (Pick next task), `add` (Add a task),
  `list` (Review status), and `update-status` when execution actually starts
  or finishes. Picking alone leaves the entry `planned`. Pick next task does
